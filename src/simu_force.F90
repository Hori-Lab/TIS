...
!$  tn = omp_get_thread_num()
  ...
  if (inmisc%class_flag(CLASS%RNA)) then
     if (inmisc%i_dtrna_model == 2013) then
        ...
        call simu_force_dtrna_hbond13(irep, force_mp_l(1,1,tn))
     else if (inmisc%i_dtrna_model == 2015) then
        call simu_force_dtrna_stack_nlocal(irep, force_mp_l(1,1,tn))
        call simu_force_dtrna_stack(irep, force_mp_l(1,1,tn))
        call simu_force_dtrna_hbond15(irep, force_mp_l(1,1,tn))
        call simu_force_exv_dt15 (irep, force_mp_l(1,1,tn))
     endif
  endif
...
!$omp end master
  ...
  if (inmisc%force_flag(INTERACT%ELE)) then
     ...
     elseif (inele%i_function_form == 1) then ! Coulomb potential
        call simu_force_ele_coulomb(irep, force_mp_l(1,1,tn))
     endif
  endif
